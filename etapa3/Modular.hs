...
-}
mapHelper :: (a -> a) -> Int -> [a] -> [a] -> Int -> a -> [[a]] -> [[a]]
mapHelper f len left right counter element result = 
    if counter == len 
        then result
        else mapHelper f len (left ++ [element]) (tail right) (counter + 1) (head right) (result ++ [(left ++ [f element]) ++ right])

mapSingle :: (a -> a) -> [a] -> [[a]]
mapSingle f [] = []
mapSingle f xs = mapHelper f (length xs) [] (tail xs) 0 (head xs) []
...
-}
partitions :: [a] -> [[[a]]]
partitions [elem] = [[[elem]]]
partitions xs = (foldl (\acc x -> ([head xs] : x) : acc) [] previousPartition) ++ (foldl (\acc x -> acc ++ (mapSingle (head xs :) x)) [] previousPartition)
    where previousPartition = partitions (tail xs)
...
helperOutNeighs :: Ord a
                => S.Set a
                -> Graph a
                -> Bool
helperOutNeighs set graph = S.size (S.map (\x -> S.filter (\y -> S.notMember y set) (outNeighbors x graph)) set) == 1

helperInNeighs :: Ord a
                => S.Set a
                -> Graph a
                -> Bool
helperInNeighs set graph = S.size (S.map (\x -> S.filter (\y -> S.notMember y set) (inNeighbors x graph)) set) == 1

isModule :: Ord a
         => S.Set a
         -> Graph a
         -> Bool
isModule set graph = (helperOutNeighs set graph) && (helperInNeighs set graph)
...
isModularPartition :: Ord a
                   ...
                   -> Graph a
                   -> Bool
isModularPartition partition graph = if S.size parts == 1 && S.member True parts then True else False
    where parts = S.map (\x -> isModule x graph) partition
...
maximalModularPartition :: Ord a
                        ...
                        -> Graph a
                        -> Partition a
maximalModularPartition partitions graph = minimumBy (compare `on` S.size) (filter (\x -> S.size x > 1 && isModularPartition x graph) partitions)
