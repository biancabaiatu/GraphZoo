...
removeNode node graph = (S.filter (\x -> x /= node) (nodes graph), S.filter (\x -> fst x /= node && snd x /= node) (edges graph))
...
splitNode old news graph = (S.union (nodes (removeNode old graph)) (S.fromList news),
                            S.union (edges (removeNode old graph)) (S.union (S.cartesianProduct (S.fromList news) (outNeighbors old graph))
...
mergeNodes prop node graph = 
    let foundNodes =  S.filter prop (nodes graph)    
    in (if (S.size (S.filter prop (nodes graph)) == 0)
                                then (nodes graph)
                                else (S.insert node (S.filter (not . prop) (nodes graph))),
                                    S.union (S.filter (\x -> (S.notMember (fst x) foundNodes) && (S.notMember (snd x) foundNodes)) (edges graph))
                                            (S.union (S.cartesianProduct (S.insert node S.empty) 
                                                                         (S.foldl (\acc x -> S.union acc (S.foldl (\acc2 y -> (if (S.member y foundNodes)
                                                                                                                                then S.insert node acc2
                                                                                                                                else S.insert y acc2)) 
                                                                                                                                S.empty (outNeighbors x graph))) 
                                                                                                                                S.empty foundNodes))
                                                     (S.cartesianProduct (S.foldl (\acc x -> S.union acc (S.foldl (\acc2 y -> (if (S.member y foundNodes)
                                                                                                                                then S.insert node acc2
                                                                                                                                else S.insert y acc2)) 
                                                                                                                                S.empty (inNeighbors x graph))) 
                                                                                                                                S.empty foundNodes) 
                                                                                                                                (S.insert node S.empty))))