...
mergeNodes prop node graph = 
    let foundNodes = S.filter prop (nodes graph)
        otherNodes = S.filter (not . prop) (nodes graph)
    in if (S.size foundNodes == 0)
                                then graph
                                else (S.union otherNodes (S.insert node S.empty), (S.foldl (\acc x -> if (S.member (fst x) foundNodes) && (S.notMember (snd x) foundNodes) 
                                                                                                        then S.insert (node, (snd x)) acc
                                                                                                      else if (S.notMember (fst x) foundNodes) && (S.member (snd x) foundNodes) 
                                                                                                        then S.insert ((fst x), node) acc
                                                                                                      else if (S.member (fst x) foundNodes) && (S.member (snd x) foundNodes) 
                                                                                                        then S.insert (node, node) acc
                                                                                                      else S.insert x acc) S.empty (edges graph)))