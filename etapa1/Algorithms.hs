...
import qualified StandardGraph as S
...
helpDFS :: Ord a
        => [a] -- lista elmentelor din stiva
        -> [a] -- lista vecinilor nodului curent
        -> [a] -- lista obtinuta prin concatenarea celor doua
helpDFS stack neighs = neighs ++ stack

helpBFS :: Ord a
        => [a] -- lista elmentelor din coada
        -> [a] -- lista vecinilor nodului curent
        -> [a] -- lista obtinuta prin concatenarea celor doua
helpBFS queue neighs = queue ++ neighs

traversal :: Ord a
        => ([a] -> [a] -> [a]) -- funcția de îmbinare a listelor de noduri
        -> Graph a             -- graful
        -> S.Set a             -- lista nodurilor vizitate
        -> [a]                 -- stiva sau coada
        -> [a]                 -- lista care contine nodurile parcurse
        -> [a]                 -- lista de noduri parcurse
traversal f graph visited neighs result = if (length neighs == 0) 
                                            then result
                                            else traversal 
                                                    f 
                                                    graph 
                                                    (S.insert (head neighs) visited) 
                                                    (f (tail neighs) (S.toList (S.filter (\x -> not (S.member x visited)) (S.outNeighbors (head neighs) graph))))
                                                    (if (S.member (head neighs) visited)
                                                        then result
                                                        else result ++ [head neighs])
...
search f node graph = traversal f graph S.empty (f [node] []) []
...
countIntermediate from to graph = 
    if ((notElem to (bfs from graph)) || (notElem to (dfs from graph)))
        then Nothing
        else Just (length (fst (span (\node -> (fromJust (L.elemIndex node (bfs from graph))) < (fromJust (L.elemIndex to (bfs from graph)))) (bfs from graph))) - 1,
                    length (fst (span (\node -> (fromJust (L.elemIndex node (dfs from graph))) < (fromJust (L.elemIndex to (dfs from graph)))) (dfs from graph))) - 1)
